...
parser = argparse.ArgumentParser(description="Starts benchmarking docker container.")
...
docker_home_dir = "/home/bench/mnt/"
shell = "/usr/bin/fish"
zombie_wrapper = "/home/bench/mnt/docker/zombie-reaper-wrapper.py"
...
def start():
    args = docker_cmd[:]
    args += ["run", "-w", docker_home_dir, "--hostname", "fuzzing-bench"]
    ...
    args += ["--ulimit", "core=0"]
    if cmd_args.ramdisk:
        args += ["--tmpfs", "/exec-tmp-ram:rw,exec,size=1g"]
    if cmd_args.ssh:
        args += ["--mount", f"type=bind,source={home}/.ssh,target=/home/bench/.ssh"]
    ...
    if len(command_to_run):
        args += ["-t"]
        args += [image_name]
        args += [zombie_wrapper, shell, "-c", " ".join(command_to_run)]
    else:
        args += ["-it"]
        # Start an interactive shell.
        args += [image_name]
        args += [zombie_wrapper, shell]
    sp.run(args)
